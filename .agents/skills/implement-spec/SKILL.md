...
For AgentDeck, read `CONTEXT.md`, `docs/architecture.md`, `docs/development.md`, and `docs/agents/issue-tracker.md` before constructing the task graph. Use `nvm use` for project commands. The final verification must include `npm test` and `npm run typecheck`, plus `npm run test:notch` for native companion changes and `npm run build` for packaging or cross-runtime changes.
