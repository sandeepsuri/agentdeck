  ...
  return (
    <article className="run-workspace">
      <header>
        ...
        <span className={`work-run-status status-${run.status}`}>{titleCase(run.status)}</span>
      </header>
      ...
      <dl className="run-intent-grid">
        ...
        <div><dt>Requested result</dt><dd>{titleCase(run.spec.requestedDeliveryResult)}</dd></div>
        <div><dt>Budget</dt><dd>{Object.entries(run.spec.budget).map(([name, value]) => <span key={name}>{titleCase(name)}: {value}</span>)}</dd></div>
