...
describe('classify', () => {
  ...
  it('is remote-but-unauthenticated when the tailnet host/token are not configured yet', () => {
    ...
    // not be denied outright before opts.remoteHost is known — but with no
    // remoteHost configured there is nothing to match, so an arbitrary host
    // is denied, not remote. This case is: remoteHost configured, token not.
    const result = classify(
      { host: `${REMOTE_HOST}:4040` },
  ...
  it('denies when no remoteHost is configured and the host is not loopback', () => {
...
describe('isLoopbackHostHeader / isAllowedOrigin (moved from app.ts, same behavior)', () => {
  ...
  it('accepts a remote-host origin only when a remoteHost is passed', () => {
